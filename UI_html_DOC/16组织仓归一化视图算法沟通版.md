...
flowchart TD
    ...
    I --> J["tissue_chart_draw_cb() 绘制竖向 16 根组织柱"]
```
...
- 后续建议算法侧直接返回 `M_i`，避免 UI 适配层继续用 `absolute_gf_percent` 反推。
...
- 背景为纯黑 `#000000`，当前采用单色绿色 G 通道亮度区分，不再使用黄/红色块。
- 每个组织仓一根竖向柱，高度读取 `tissue_bar_permille[i]`。
- 400 位置画环境压力横线，颜色 `#007F00`；`TISSUE_SHOW_AMB_LABEL` 打开时显示 `AMB`。
- 900 位置画 M 值横线，颜色 `#00FF00`；`TISSUE_SHOW_M_LABEL` 打开时显示 `M`。
- `tissue_pi_permille` 位置画 PI 横向虚线，颜色 `#003300`。
- `TISSUE_SHOW_PI_LABEL`、`TISSUE_SHOW_AMB_LABEL`、`TISSUE_SHOW_M_LABEL` 只控制文字标签是否显示，不改变参考线位置和条长计算。
- 组织柱低于 400 的部分使用安全区暗场色 `#004C00`；400~900 的部分使用排氮高亮色 `#00CC00`；高度超过 900 时，900 以上的部分使用满亮 `#00FF00` 闪烁。

## 自定义组件 Leading Tissue 临时口径

`COMP_TISSUE_GF_4012` 和 `COMP_TISSUE_RAW_4012` 当前先在 UI 侧临时提取主导组织，后续建议算法层直接输出这些字段。

当前 UI 侧使用现有 VM 字段：

```c
float tissue_ambient_pressure_bar;  // P_amb
float tissue_n2_bar[16];            // PN2_i
float tissue_m_value_bar[16];       // M_i，当前 PC 适配层临时反推
...
if PN2_i <= P_amb:
    ...
    GF_i = ((PN2_i - P_amb) / (M_i - P_amb)) * 100

leading_idx = index of max(GF_i)
```

平局处理：如果 16 个组织都处于 `PN2_i <= P_amb`，即 `GF_i` 全为 0，UI 临时选择 `PN2_i` 最大的组织作为 `leading_idx`，用于让 `TISSUE(RAW)` 仍显示最接近环境压力的组织压力。

两个组件当前显示：

- `TISSUE(GF)`：显示 `GF_max` 的 0..100 水平进度条；`GF_max >= 100` 时满亮绿色闪烁。
- `TISSUE(RAW)`：显示 `PN2_leading` 的 0..6.0 bar 水平压力条；`0..P_amb` 为暗绿色，`P_amb..PN2` 为满亮绿色，并绘制 `P_amb` 和 `M_idx` 两条竖线。

注意：这个 leading 口径当前只用于两个自定义小组件，不改变 DECO 主图 16 根组织柱。DECO 主图仍按 `PN2 + PHe` 做归一化条长；这两个小组件按照本次需求先使用 `PN2` 提取 leading。若算法侧认为 Trimix 场景下 leading 也必须使用总惰性气体压力 `PN2 + PHe`，请明确给出正式字段和公式口径。

## 建议算法侧提供的正式接口

这版实现是为了快速验证 UI 表达。当前 PC 适配层已经能画出目标视图，但 `M_i` 仍然依赖 `absolute_gf_percent` 临时反推。为了让 PC 模拟器和真机固件长期保持同一物理口径，建议算法库正式提供组织仓压力指标，UI 适配层继续负责 `0..1000` 的显示映射。

建议接口形态：

```c
typedef struct {
    float ambient_pressure_bar;          // P_amb
    float inspired_n2_bar;               // P_I
    float tissue_n2_bar[16];             // PN2_i
    float tissue_he_bar[16];             // PHe_i
    float tissue_m_value_bar[16];        // M_i
} tissue_pressure_metrics_t;
```

如果算法侧不希望 UI 计算 Leading Tissue，建议额外直接提供：

```c
typedef struct {
    uint8_t leading_compartment;          // 0..15
    float leading_gf_percent;             // GF_max
    float leading_pn2_bar;                // PN2_leading
    float leading_total_inert_bar;        // PN2 + PHe，可选但建议提供
    float leading_m_value_bar;            // M_idx
    float ambient_pressure_bar;           // P_amb
} tissue_leading_metrics_t;
```

这样 `COMP_TISSUE_GF_4012` 可直接消费 `leading_gf_percent`，`COMP_TISSUE_RAW_4012` 可直接消费 `leading_pn2_bar / ambient_pressure_bar / leading_m_value_bar`，UI 不再遍历 16 个组织仓计算主导组织。

字段语义：

- `ambient_pressure_bar`：当前环境压力 `P_amb`。
- `inspired_n2_bar`：当前吸入氮气分压 `P_I`。
- `tissue_n2_bar[16]`：每个组织仓氮气分压 `PN2_i`。
- `tissue_he_bar[16]`：每个组织仓氦气分压 `PHe_i`。
- `tissue_m_value_bar[16]`：每个组织仓当前环境压力下的 M 值上限 `M_i`。

UI 适配层会基于这些物理量自行计算显示字段：
...
if P_tissue_i <= P_amb:
    ...
    bar_i = 400 + ((P_tissue_i - P_amb) / (M_i - P_amb)) * (900 - 400)

PI_permille = (P_I / P_amb) * 400
```

这样算法库只负责输出物理真相，UI 适配层负责把物理量映射成当前产品的显示坐标。后续如果 UI 要调整 `400/900` 锚点、颜色、标签或坐标表达，不需要修改算法库。
...
4. 算法侧是否可以直接输出每仓 `M_i`，从而删除当前 PC 适配层用 `absolute_gf_percent` 反推 `M_i` 的临时逻辑？
5. UI 固定 400 为环境压力线、900 为 M 值线是否符合算法侧对风险区间的解释？

## 当前验证点

- `bus_get_tissue_normalized_valid()` 为 true 后，DECO 主图应显示竖向 16 根组织柱。
...
- 当 `P_tissue_i > M_i` 时，条长超过 900，UI 应显示满亮绿色闪烁段。
