...
- 组织条背景按安全区/警示区/危险区分段显示，当前横向回归版本使用绿色、黄色、红色三段。
- 每个组织仓一条横向组织条，长度读取 `tissue_bar_permille[i]`。
- 400 位置画环境压力竖线，颜色 `#00FF00`，底部显示 `PAMB`。
- 900 位置画 M 值竖线，颜色 `#FF2020`，底部显示 `M`。
- `tissue_pi_permille` 位置画 PI 竖向虚线，颜色 `#55FF55`，底部显示 `PI`。
- `TISSUE_SHOW_PI_LABEL`、`TISSUE_SHOW_AMB_LABEL`、`TISSUE_SHOW_M_LABEL` 只控制文字标签是否显示，不改变参考线位置和条长计算。
- 组织条低于 400 的部分使用安全区绿色；400~900 的部分使用警示黄色；超过 900 的部分使用危险红色闪烁。
...
注意：这个 leading 口径当前只用于两个自定义小组件，不改变 DECO 主图 16 行组织条。DECO 主图仍按 `PN2 + PHe` 做归一化条长；这两个小组件按照本次需求先使用 `PN2` 提取 leading。若算法侧认为 Trimix 场景下 leading 也必须使用总惰性气体压力 `PN2 + PHe`，请明确给出正式字段和公式口径。
...
- 当 `P_tissue_i > M_i` 时，条长超过 900，UI 应显示危险红色闪烁段。
