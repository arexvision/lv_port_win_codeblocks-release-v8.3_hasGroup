...
#define TISSUE_OPA_SAFE          LV_OPA_40
#define TISSUE_OPA_OVER_MIN      LV_OPA_50
#define TISSUE_OPA_OVER_MAX      LV_OPA_100
#define TISSUE_OPA_PI            LV_OPA_40
```

绘制行为：

- 组织条不再使用绿色、黄色、红色三段色相区分；当前横向回归版本统一使用纯绿，只通过 opacity 表达状态。
- 每个组织仓一条横向组织条，长度读取 `tissue_bar_permille[i]`。
- 400 位置画环境压力竖线，纯绿 `LV_OPA_40`，底部显示 `PAMB`。
- 900 位置画 M 值竖线，纯绿 `LV_OPA_100`，底部显示 `M`。
- `tissue_pi_permille` 位置画 PI 竖向虚线，纯绿低透明度，底部显示 `PI`。
- `TISSUE_SHOW_PI_LABEL`、`TISSUE_SHOW_AMB_LABEL`、`TISSUE_SHOW_M_LABEL` 只控制文字标签是否显示，不改变参考线位置和条长计算。
- 组织条低于 400 的安全段使用 `LV_OPA_40`；400~900 的排氮段根据接近 M 值的比例在 `LV_OPA_50..LV_OPA_100` 之间线性映射；超过 900 的部分使用纯绿满亮闪烁。
...
- Leading Tissue 小组件在 UI 层按 `PN2 + PHe`、`P_amb` 和 `M_i` 计算最大 GF；如果后续算法直接输出 leading summary，可再把这段提取逻辑迁移到算法接口。
...
3. 如果后续需要算法直接输出 Leading Tissue summary，字段建议为 `leading_compartment`、`leading_gf_percent`、`leading_tissue_inert_bar`、`leading_m_value_bar` 和 `leading_m_gf_bar`。
