...
#define TISSUE_SHOW_PI_LABEL      1
#define TISSUE_SHOW_AMB_LABEL     1
#define TISSUE_SHOW_M_LABEL       1
```

绘制行为：

- 背景固定分三段：`0..400`、`400..900`、`900..1000`，当前采用单色绿色亮度区分，不再使用黄/红色块。
- 每个组织仓一根竖向柱，高度读取 `tissue_bar_permille[i]`。
- 400 位置画环境压力横线，UI 标签显示为 `AMB`。
- 900 位置画 M 值横线 `M`。
- `tissue_pi_permille` 位置画 PI 横向虚线。
- `TISSUE_SHOW_PI_LABEL`、`TISSUE_SHOW_AMB_LABEL`、`TISSUE_SHOW_M_LABEL` 只控制文字标签是否显示，不改变参考线位置和条长计算。
- 组织柱低于 400 的部分低亮显示，高于 400 的部分高亮显示；高度超过 900 时，900 以上的部分进入高亮闪烁。
