...
## 29. 左侧 2x6 绝对网格布局重构 (v2026-04-28)

### 29.1 核心设计原则

**彻底废弃 `current_y` 累加排版，改为 `x*y*w*h` 纯数学坐标推演**

左侧 160x360 区域（不含底部 60px SystemData）被严格划分为 **2 (80px) x 6 (60px)** 的绝对网格矩阵，彻底与右侧 `arex_widget_id_t` 体系共享同一数据源。

```
Grid Layout:
  Row 0:  NDL           |           (2x1 -> 160x60)
  Row 1-2: DEPTH (x2)   |           (2x2 -> 160x120, 含速度小图标)
  Row 3:  TIME          |           (2x1 -> 160x60, 潜水时间 MM:SS)
...
**`arex_custom_widget_cfg_t`**（定义于 `arex_ui_engine.h`）：
...
typedef struct {
    arex_widget_id_t widget_id;  /* 组件类型 ID（复用 arex_widget_id_t） */
    uint8_t x;                  /* 列索引 0~1 */
    uint8_t y;                  /* 行索引 0~5 */
    uint8_t w;                  /* 跨越列数 1~2 */
    uint8_t h;                  /* 跨越行数 1~2 */
...
```
纯数学 cell_w * cell_h 坐标推演：
  abs_x = cfg->x * 80
  abs_y = cfg->y * 60
  abs_w = cfg->w * 80
  abs_h = cfg->h * 60

内部调用 render_widget_by_id(parent, w_id, abs_x, abs_y, abs_w, abs_h, cfg->w, cfg->h, is_depth_icon)
```
    ...
    for (uint8_t i = 0; i < g_left_widget_count; i++) {
        ...
        switch (w_id) {
            case AREX_WIDGET_NDL:
                arex_widget_set_value(AREX_WIDGET_NDL, (float)g_sensor_data.ndl); break;
            case AREX_WIDGET_DEPTH:
                arex_widget_set_value(AREX_WIDGET_DEPTH, g_sensor_data.depth); break;
            case AREX_WIDGET_WTIME:
                arex_widget_set_value(AREX_WIDGET_WTIME, (float)g_sensor_data.dive_time_s); break;
            case AREX_WIDGET_GAS:
                arex_widget_set_text(AREX_WIDGET_GAS, g_sensor_data.gas_name); break;
            case AREX_WIDGET_POD1:
                arex_widget_set_value(AREX_WIDGET_POD1, g_sensor_data.pod1_bar); break;
            case AREX_WIDGET_POD2:
                arex_widget_set_value(AREX_WIDGET_POD2, g_sensor_data.pod2_bar); break;
            // ... 其他 widget 同样处理
            default: break;
        }
    }
...
{
  "left_widgets": [
    { "widget_id": 6,  "x": 0, "y": 0, "w": 2, "h": 1, "font_id": 2 },
    { "widget_id": 1,  "x": 0, "y": 1, "w": 2, "h": 2, "font_id": 3 },
    { "widget_id": 12, "x": 0, "y": 3, "w": 2, "h": 1, "font_id": 2 },
    { "widget_id": 13, "x": 0, "y": 4, "w": 2, "h": 1, "font_id": 2 },
    { "widget_id": 10, "x": 0, "y": 5, "w": 1, "h": 1, "font_id": 0 },
    { "widget_id": 11, "x": 1, "y": 5, "w": 1, "h": 1, "font_id": 0 }
  ]
}
```
