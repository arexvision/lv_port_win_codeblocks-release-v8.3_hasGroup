...
> - 告警清除逻辑重构：速度降到安全范围后自动清除，但最少显示 5 秒
> - 速率图标静止闪烁修复：静止时显示 level0 不闪烁
> - 编译错误：`lv_tick_elapsed` → `lv_tick_elaps`（LVGL v8.3 API）
>
> 详细修改记录请参见 [CHANGELOG_20260503.md](./CHANGELOG_20260503.md)

> **2026-05-03 更新 #4**：详细修改记录请参见 [CHANGELOG_20260503.md](./CHANGELOG_20260503.md)
>
> **2026-05-03 更新 #3**：`ui_engine.h` 移除重复注释

> **2026-05-03 更新 #2**：
> - 告警横幅系统重构：`arex_show_alarm_banner()` 改为在 Safe Zone 内部顶部显示，修复了横幅被遮挡的问题
> - 新增 `arex_get_safe_zone()` 函数（`screen.c`），允许外部模块获取 Safe Zone 容器对象
...
├── screen.h/c          # LVGL 控件树创建 + 所有屏幕操作 API
├── arex_input.h/c           # 输入事件捕获（键盘/编码器 → 状态机）
├── card_registry.h/c # 卡片注册表（ID、title、create/update 回调）
├── data.h/c            # 数据总线头文件存根（ui_engine.h 包含一切）
└── cards/
    ...
    └── card_setup.c         # 5F: DIVE MENU（6 条静态列表）
```
...
- 气体常量 `AREX_GAS_NAMES[]` / `AREX_GAS_MOD_M[]` 定义于 `ui_engine.c`
...
```
UI_SUB_MENU（DIVE MENU 子菜单，"MOD PO2: X.X" 行高亮）
  │
...
| 7 CARD_ID_SETUP | card_setup.c | DIVE MENU | `arex_render_dynamic_menu()` 渲染，`arex_menu_item_cfg_t[]` 含 badge 徽章 |
...
> 旧版 `AREX_FONT_*` 宏定义仅在 `screen.h` 中保留兼容层，**禁止在新代码中使用**。
...
1. **卡片不直接写状态**：card_*.c 只读 `g_sensor_data` 和 `g_ui`，不修改它们。状态修改统一在 `ui_state.c` 中进行。

2. **screen 层是哑的**：`screen.c` 的函数只负责操作控件，业务判断（如气体深度校验）在状态机里完成。
...
4. **注册回调**：card_info.c 和 card_setup.c 通过 `arex_screen_register_info_list()` / `arex_screen_register_setup_list()` 把它们内部创建的列表对象告知 screen 层，避免在 screen.c 中重复创建控件。
...
7. **数据总线归一化**：`data.h` 仅作存根（`#include "ui_engine.h"`），所有数据总线（`g_sys_config`、`g_sensor_data`）、枚举、宏均统一在 `ui_engine.h` 中定义，消除跨文件依赖。

---

## 14. 子菜单动作路由（v0.10 新增）

`arex_screen_handle_submenu_select()` 在 `screen.c` 中全面实现，按 `cur_title` 分支路由：
...
### 14.5 DIVE MENU 嵌套菜单中 MOD PO2 实时值
...
| 2026-04-23 | `ui_engine.h` | 删除 `left_order[]` 和 `left_mod_*[]`，替换为 `arex_left_row_cfg_t left_layout[8]` 行配置结构体 |
| 2026-04-23 | `ui_engine.h` | 新增 `AREX_MODULE_EMPTY=0`（替换旧 NONE），新增 `AREX_MAX_LEFT_ROWS=8`、`AREX_ROW_MAX_SLOTS=2`、`ANCHOR_COMP_COUNT=16` |
| 2026-04-23 | `ui_engine.c` | `arex_calc_anchor_layout()` 完全重写：遍历 `left_layout[]` 而非 `left_order[]`，单栏独占全宽(160px)，双拼各半宽(80px)，零双拼硬编码 |
| 2026-04-23 | `ui_engine.c` | `arex_sys_config_defaults()` 填充 `left_layout[]` 默认行配置（DEPTH/GA + NDL+TTS + POD1+POD2 + BATT+WTM + GAS + TIME） |
| 2026-04-23 | `screen.c` | `left_anchor_create()` 循环改为 `for (i < comp_count)`，标题和数值文字改为 `switch (c->module)` 枚举驱动 |
| 2026-04-23 | `screen.c` | `left_anchor_rebuild()` 同步更新为空模块检查和对齐处理 |
| 2026-04-23 | `AREX_ARCH.md` | Section 16 全面升级：新增 16.3 `arex_left_row_cfg_t` 结构体、16.4 默认行布局、16.9 渲染流程图 |
| 2026-04-23 | `ui_engine.h` | 新增 `arex_font_id_t` 枚举字典、`arex_get_font()` 声明；删除废弃的 `align_title/huge/med` 字段 |
| 2026-04-23 | `ui_engine.c` | 实现 `arex_get_font()` 字体映射器；删除废弃的 `font_sz_*` 默认值；更新 `def_layout[]` 使用 `AREX_FONT_ID_*` 枚举 |
| 2026-04-23 | `screen.c` | 全部 `AREX_FONT_*` 宏替换为 `arex_get_font(id)`；删除 `font_cat[]` 中间层数组（两处）；`left_anchor_rebuild()` 增加 `title_font`/`title_align` 填充 |
| 2026-04-23 | `cards/*.c` | 全部 `AREX_FONT_*` 宏替换为 `arex_get_font(id)`（compass/setup/gas/deco/info/plan） |
| 2026-04-23 | `screen.h` | 旧 `AREX_FONT_*` 宏标记为废弃，附正确用法注释 |
| 2026-04-23 | `AREX_ARCH.md` | 新增 Section 17 字体映射引擎文档；更新 Section 10/16 引用 |
| 2026-04-23 | `ui_engine.h` | 新增 `arex_menu_item_cfg_t` 及 `arex_render_dynamic_menu()` 声明 |
| 2026-04-23 | `ui_engine.c` | 实现工厂函数，height/gap 全程从 `g_sys_config` 推算 |
...
|| 2026-04-23 | `ui_engine.h` | 新增 `AREX_DEBUG_BORDER` 宏(0=关闭/1=开启)，统一控制 title_zone/val_zone 调试边框；`RENDER_FIXES.md` Section 7 同步更新 |
...
| 函数 | 文件 | 作用 |
|------|------|------|
| `arex_calc_anchor_layout()` | ui_engine.c | 遍历 `left_layout[]`，填 `arex_anchor_comp_t[]`（单栏1入口，双拼2入口），返回实际 count |
| `arex_calc_tech_layout()` | ui_engine.c | Tech 模式左右分区坐标：左锚点固定 160px，右区域 `= safe_zone_w - 160 - gap` |
| `arex_calc_classic_layout()` | ui_engine.c | Classic 模式上下分区，最小高度 `AREX_MIN_CLASSIC_TOP_H=200px` |
| `arex_calc_widget_cell()` | ui_engine.c | 5x6 网格单元格坐标，`unit_w = parent_w/5`，`unit_h = parent_h/6` |
| `arex_calc_tissue_bars()` | ui_engine.c | 16 柱组织图 X 坐标，`col_w = total_w/16` |
| `left_anchor_create()` | screen.c | 首次创建，按 `c->module` 枚举驱动，无索引硬编码 |
| `left_anchor_rebuild()` | screen.c | 配置变更后重建，数据驱动字体/对齐刷新 |
| `right_panel_create()` | screen.c | 创建 tileview，按 `card_order[]` 顺序挂载卡片 |
...
字体映射器是全系统中**唯一**允许将字体 ID 转换为真实 `lvgl` 字体指针的地方，位于 `ui_engine.c`：

```c
/* 声明字体资源（由 fonts.h 中的 LV_FONT_DECLARE 提供） */
...
`screen.h` 中保留了 4 个旧的 `AREX_FONT_*` 宏作为兼容层，**新代码禁止使用**：
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-04-23 | `ui_engine.h` | 新增 `arex_font_id_t` 枚举字典；`arex_get_font()` 声明 |
| 2026-04-23 | `ui_engine.c` | 实现 `arex_get_font()` 映射器；引入 `fonts/fonts.h`；删除废弃的 `font_sz_*` 字段和默认值 |
| 2026-04-23 | `ui_engine.h` | 删除 `arex_sys_config_t` 中废弃的 `align_title/huge/med` 字段 |
| 2026-04-23 | `screen.c` | 所有 `AREX_FONT_*` 宏替换为 `arex_get_font(id)`；删除 `font_cat[]` 中间层数组（两处） |
| 2026-04-23 | `cards/*.c` | 所有卡片中 `AREX_FONT_*` 宏替换为 `arex_get_font(id)` |
| 2026-04-23 | `screen.h` | 旧宏标记为废弃，附正确用法注释 |
...
```
create 阶段:
  card_registry.c: s_registry[i].tile_obj = NULL（初始化）
...
| 枚举 / 宏 | 文件 | 说明 |
|------|------|------|
| `arex_card_pos_t` | card_registry.h | 位置枚举：CARD_POS_INFO=0(固定), CARD_POS_1~4(可重排), CARD_POS_SETUP=5(固定) |
| `arex_card_id_t` | card_registry.h | 卡片固有身份枚举：CARD_ID_INFO ~ CARD_ID_SETUP |
| `AREX_CARD_COUNT` | card_registry.h | 卡片总数 = 8 |
| `AREX_DASH_CARD_COUNT` | card_registry.h | DASH 可滑动数 = 6 |

| 函数 | 文件 | 作用 |
|------|------|------|
| `arex_card_count()` | card_registry.c | 返回卡片总数 |
...
单张表管理所有卡片信息，新增卡片只需在 `g_cards[]` 加一条，无需修改 `screen.c`。
...
| 文件 | 职责 |
|------|------|
| `card_registry.h` | `arex_card_engine_t` + `arex_card_t` 类型定义 + API 声明 |
| `card_registry.c` | `g_cards[]` 单张统一表（ROM 字段 + 运行时 tile_obj） |
| `ui_engine.h` | `arex_menu_item_cfg_t` + `arex_menu_list_cfg_t` 配置结构体 |
| `card_info.c` | 暴露 `info_menu_cfg`，`create_cb` 内注册 `s_info_list` |
| `card_setup.c` | 暴露 `setup_menu_cfg`，`create_cb` 内注册 `s_setup_list` |
| `screen.c` | `right_panel_create()` 引擎分发循环 |
...
Step 2: 在 ui_engine.c 的 g_card_registry[] 之前加静态配置变量
    static const arex_card_menu_cfg_t s_cfg_camera = { g_camera_item_count, g_camera_items };

Step 3: 在 ui_engine.c 的 g_card_registry[] 中加一行
    [CARD_ID_CAMERA] = {
        .engine_type = CARD_ENGINE_MENU,
        .title      = "6F: CAMERA",
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-04-24 | `ui_engine.h` | 新增 `AREX_CARD_TITLE_H` 常量(40px)；新增 `arex_calc_widget_grid()` 公开声明 |
| 2026-04-24 | `ui_engine.c` | 重构 `arex_calc_widget_grid()`：移除 parent_x/y，锁定 80x60 基准，增加 40px 标题避让偏移 |
...
**修复 2：`screen.c` 中 `left_anchor_create()` 中 `AREX_MODULE_TIME` 改用数据总线**
...
```c
// ui_engine.c:513-514 初始值归零
g_sensor_data.pod1_bar = 0.0f;
g_sensor_data.pod2_bar = 0.0f;

// screen.c left_anchor_create() / arex_screen_refresh_left_panel()
// 渲染时判断 0 显示 "--"
...
| 2026-04-24 | `screen.c` | `AREX_MODULE_TIME` 创建时从硬编码 `"00:00"` 改为 `g_sensor_data.dive_time_s` |
| 2026-04-24 | `ui_engine.c` | `pod1_bar / pod2_bar` 初始值从 `210.0f/195.0f` 改为 `0.0f` |
| 2026-04-24 | `screen.c` | POD1/POD2 渲染（创建+刷新+INFO菜单）加 `0.0f = "--"` 判断 |
| 2026-04-24 | `AREX_ARCH.md` | 新增 Section 24 记录本次修复 |
| 2026-04-24 | `ui_engine.h` | `arex_dive_pt_t.time_min` 改为 `time_s`（统一秒级单位） |
| 2026-04-24 | `card_plan.c` | 重写 `plan_chart_draw_cb` 为秒级坐标引擎；`init_test_data()` 清零 `g_dive_log_count`；新增 `arex_dive_log_append()` |
| 2026-04-24 | `data.h` | 新增 `arex_dive_log_append()` 声明 |
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-04-24 | `ui_engine.h` | `arex_dive_pt_t.time_min` 改为 `time_s` |
...
### 26.3 Data Bus Setter 接口（`data.h / data.c`）
...
| 2026-04-24 | `AREX_ARCH.md` | 新增 Section 26 |
| 2026-04-28 | `data.c` | `arex_bus_set_battery/temperature` 加首次强制打脏逻辑，防止 placeholder "--" 无法被替换 |
| 2026-04-28 | `ui_engine.h` | `AREX_SHOW_PLACEHOLDER_ON_INIT` 宏声明 |
| 2026-04-28 | `ui_engine.c` | `render_widget_by_id` 中 DEPTH/NDL/通用渲染分支全部加 `AREX_SHOW_PLACEHOLDER_ON_INIT` 统一控制 |
...
**统一宏 `AREX_CARD_TITLE_H`**（定义在 `ui_engine.h`，值为 40px）：
...
| 文件 | 变更 |
|------|------|
| `ui_engine.h` | 注释更新为"右侧卡片全局标题区高度分配" |
| `screen.c` | `arex_screen_make_card_title()` 标题 Y=8, h=30, 分割线 Y=`AREX_CARD_TITLE_H-2` |
| `screen.c` | 子菜单标题 Y=8, 分割线 Y=`AREX_CARD_TITLE_H-2`, 列表 Y=`AREX_CARD_TITLE_H` |
...
| `ui_engine.c` | 5F 网格标题文字 Y=8, h=`AREX_CARD_TITLE_H-10`, 分割线 Y=`AREX_CARD_TITLE_H-2`（原 38）；更新注释 |
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-04-25 | `ui_engine.h` | `AREX_CARD_TITLE_H` 注释更新 |
| 2026-04-25 | `screen.c` | 标题区全部 Y 值改用宏 |
...
| 2026-04-25 | `ui_engine.c` | 5F 网格分割线 Y 改用宏 |
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-04-26 | `ui_engine.h` | 新增 `arex_render_card_title()` 声明 |
| 2026-04-26 | `ui_engine.c` | 实现 `arex_render_card_title()`；重构 `arex_calc_widget_grid()` 动态 cell_h |
| 2026-04-26 | `ui_engine.c` | `arex_render_5f_custom_grid()` 改用 `arex_render_card_title()` |
...
Grid Layout:
  ...
  Row 3:  DIVE TIME     |           (2x1 -> 160x60, 潜水时间 MM:SS)
  Row 4:  GAS           |           (2x1 -> 160x60, 当前气体名称)
  Row 5:  POD1         | POD2     (1x1 -> 80x60, 双拼)
  Row 6:  SYS           |           (2x1 -> 160x60, 系统状态，含电池条)
  [底部 60px SystemData 独立渲染：电池/温度/设备图标]
```

### 29.2 新增数据结构

**`arex_widget_id_t` 枚举扩展**（`ui_engine.h`）：

> **Protobuf 对齐原则**：枚举值与 APP 端 `WidgetType` 完全一致，不存在的 ID 已在 MCU 侧移除。`WIDGET_WTIME_0806`（水面休息时间）已废弃，替换为 `WIDGET_TIME_1606`（系统时间）和 `WIDGET_DIVE_TIME_1606`（潜水时间）。
...
typedef enum {
    ...
    WIDGET_DEPTH_1612       = 2,
    WIDGET_DEPTH_1606      = 3,
    WIDGET_DIVE_TIME_1606  = 4,  /* 潜水时间 MM:SS — 数据源: g_sensor_data.dive_time_s */
    WIDGET_GAS_1606        = 5,  /* 当前气体 — 数据源: g_sensor_data.gas_name */
    WIDGET_SYS_1606        = 6,  /* 系统时间 — 数据源: g_sensor_data.sys_time_h/m */

    /* --- 基础组件 (Basic) --- */
    WIDGET_TEMP_0806        = 10,
    WIDGET_TIME_1606        = 11, /* 系统时间 MM:SS — 数据源: g_sensor_data.sys_time_h/m */
    WIDGET_TTS_0806         = 12,

    /* --- 技术潜水 (Tech Dive) --- */
    WIDGET_SURF_GF_0806    = 20,
    WIDGET_GF99_0806        = 21,
    WIDGET_TISSUE_GF_4012  = 22,
    WIDGET_TISSUE_RAW_4012 = 23,
    WIDGET_CNS_0806         = 24,
    WIDGET_OTU_0806         = 25,
    WIDGET_ASCENT_0806     = 26,
    WIDGET_ASCENT_0812     = 27,
    WIDGET_STOP_DEPTH_0806 = 28,
    WIDGET_STOP_TIME_1606  = 29,
    WIDGET_COMPASS_1612    = 30,
    WIDGET_COMPASS_0808    = 31,
     ...
     *   WIDGET_TEMP_MAX_0806  = 38  (原最大温度)
     *   WIDGET_SAC_RATE_0806  = 39  (原SAC呼吸速率)
     *   WIDGET_PPO2_SAFE_0806 = 50  (原PPO2安全值)
     *   WIDGET_NDL_SAFE_0806  = 51  (原NDL安全值)
     *   WIDGET_SAC_SAFE_0806  = 52  (原SAC安全值)
     *   (WIDGET_WTIME_0806    = 40  已废弃，替换为 TIME_1606 + DIVE_TIME_1606)
     */
} arex_widget_id_t;
```

**`arex_left_widget_t`**（定义于 `ui_engine.h`）：
...
**`ui_engine.c` 中 `arex_sys_config_defaults()` 填充**：

```c
g_left_widget_count = 7;
g_left_widgets[0] = (arex_left_widget_t){ WIDGET_NDL_STOP_1606,   0, 0 };  /* 免减压时间 */
g_left_widgets[1] = (arex_left_widget_t){ WIDGET_DEPTH_1612,      0, 1 };  /* 深度 2x2 */
g_left_widgets[2] = (arex_left_widget_t){ WIDGET_DIVE_TIME_1606,   0, 3 };  /* 潜水时间 */
g_left_widgets[3] = (arex_left_widget_t){ WIDGET_GAS_1606,       0, 4 };  /* 当前气体 */
g_left_widgets[4] = (arex_left_widget_t){ WIDGET_POD_0806,       0, 5 };  /* POD1 */
g_left_widgets[5] = (arex_left_widget_t){ WIDGET_POD_0806,       1, 5 };  /* POD2 */
g_left_widgets[6] = (arex_left_widget_t){ WIDGET_SYS_1606,       0, 6 };  /* 系统状态 */
```

> **Row 3 说明**：`WIDGET_WTIME_0806` 已移除，替换为 `WIDGET_DIVE_TIME_1606`（潜水时间 MM:SS）。`WIDGET_TIME_1606`（系统时间 MM:SS）保留在样式表中，可按需布局。

### 29.4 网格渲染引擎

**`arex_render_left_anchor_grid(lv_obj_t *left_anchor)`**（`ui_engine.c`）：
...
| Widget ID | 格式化 | 示例 | 接口 |
|-----------|--------|------|------|
| `DEPTH` / `TEMP` | `%.1f` | `45.2` | `arex_widget_set_value` |
| `PPO2` | `%.2f` | `1.20` | `arex_widget_set_value` |
| `POD1` / `POD2` | `%.0f` | `210` | `arex_widget_set_value` |
| `DIVE_TIME_1606` | `%02d:%02d` | `05:32` | `arex_widget_set_text` |
| `TIME_1606` | `%02d:%02d` | `14:30` | 通用流水线 |
| `SYS_1606` | `%02d:%02d` | `14:30` | 通用流水线 |
| `BATTERY` | `%.0f%%` | `85%` | `arex_widget_set_value` |
| `TTS` / `NDL` | `%d` | `24` | `arex_widget_set_value` |
| 其他 | `%.0f` | - | `arex_widget_set_value` |

> **注意**：`WIDGET_DIVE_TIME_1606` 使用 `arex_widget_set_text()` 而非 `arex_widget_set_value()`，因为 `arex_widget_set_value()` 会将浮点数格式化为整数，无法满足 `MM:SS` 时间格式需求。

**`arex_widget_set_text(arex_widget_id_t id, const char *text)`**：

用于非数值组件（如 GAS 气体名称）和需要自定义格式化的时间组件（DIVE_TIME）。同样双向搜索容器，命中即更新 label 文字。

### 29.7 左侧面板刷新 — 数据源自动推导

**`arex_screen_refresh_left_panel()`（`screen.c`）**：
...
arex_bus_set_dive_time(dive_s)
    → data.c: g_sensor_data.dirty_mask |= DIRTY_DIVE_TIME
    ↓
ui_engine.c: update_task_timer_cb()
    → if (mask & DIRTY_DIVE_TIME) → arex_screen_refresh_left_panel()
    ↓
screen.c: arex_screen_refresh_left_panel()
    → switch(w_id) case WIDGET_DIVE_TIME_1606
    → arex_widget_set_text(WIDGET_DIVE_TIME_1606, "MM:SS")
    ↓
ui_engine.c: arex_widget_set_text()
...
> **协议对齐说明**：`WIDGET_WTIME_0806`（水面休息时间）已废弃，对应 widget_id=40 不再下发。APP 应使用 `WIDGET_TIME_1606`（系统时间）和 `WIDGET_DIVE_TIME_1606`（潜水时间）。

```json
{
  "left_widgets": [
    { "widget_id": 1,  "x": 0, "y": 0, "w": 2, "h": 1 },  /* NDL */
    { "widget_id": 2,  "x": 0, "y": 1, "w": 2, "h": 2 },  /* DEPTH */
    { "widget_id": 4,  "x": 0, "y": 3, "w": 2, "h": 1 },  /* DIVE_TIME */
    { "widget_id": 5,  "x": 0, "y": 4, "w": 2, "h": 1 },  /* GAS */
    { "widget_id": 33, "x": 0, "y": 5, "w": 1, "h": 1 },  /* POD1 */
    { "widget_id": 33, "x": 1, "y": 5, "w": 1, "h": 1 }   /* POD2 */
  ]
}
```
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-04-28 | `ui_engine.h` | 新增 `AREX_LEFT_COLS/ROWS/CELL_W/CELL_H/GRID_W/GRID_H` 宏；新增 `arex_custom_widget_cfg_t` 结构体；新增 `arex_render_left_anchor_grid()` 与 `render_widget_by_id()` 声明 |
| 2026-04-28 | `ui_engine.c` | 全局 `g_left_widgets[]` / `g_left_widget_count` 定义；`arex_sys_config_defaults()` 填充默认网格配置；`arex_render_left_anchor_grid()` 实现；`render_widget_by_id()` 重写（含字号自适应内联）；`arex_widget_set_value()` 改为双容器搜索；新增 `arex_widget_set_text()` 字符串刷新；新增 `arex_render_system_data()` 底部 60px SystemData 渲染 |
| 2026-04-28 | `screen.c` | `left_anchor_create()` 删除组件创建循环；`left_anchor_rebuild()` 简化为清空 + 调用两个渲染函数；`arex_screen_refresh_left_panel()` 改为遍历 `g_left_widgets[]` 自动推导数据源；删除 `s_anchor_titles[]` / `s_anchor_vals[]` / `s_anchor_seps[]` / `s_anchor_mod_seps[]` 相关逻辑 |
| 2026-04-28 | `AREX_ARCH.md` | 新增 Section 29 |
| 2026-04-28 | `ui_engine.h` | `WIDGET_GAS_1606` 新增；`WIDGET_WTIME_0806` 注释改为 DIVE_TIME，数据源改为 `dive_time_s` |
| 2026-04-28 | `ui_engine.c` | `s_widget_meta[]` 新增 GAS 条目；WTIME title 改为 "DIVE" |
| 2026-04-28 | `screen.c` | `arex_screen_refresh_left_panel()` 重构为 switch-case 路由，遍历 `g_left_widgets[]` 自动推导数据源；移除 TTS；新增 GAS 字符串刷新 |
| 2026-04-28 | `ui_engine.c` | `render_widget_by_id()` 新增三大改进：(1) `LV_OBJ_FLAG_SCROLLABLE` + `LV_SCROLLBAR_MODE_OFF` 封杀滚动条；(2) `AREX_WIDGET_DEPTH` 专属渲染：整数(HUGE)+小数(MEDIUM)+单位(SMALL)+箭头四层分离排版；(3) `AREX_WIDGET_NDL` 专属渲染：电池型 Bar(左)+巨型数值(中)+NDL标签(右)横向布局 |
| 2026-05-01 | `data.h` | `ble_sync_left_widget_t` 删除 `w/h` 字段；BLE 帧总大小由 172 字节缩减为 148 字节 |
| 2026-05-01 v2 | `ui_engine.h` | `arex_left_widget_t` 移除 `w/h` 字段，统一由 `arex_get_widget_style()` 查表获取 |
| 2026-05-01 v2 | `ui_engine.c` | `arex_render_left_anchor_grid()` 改为从样式表查表获取 span_w/span_h |
| 2026-05-01 | `ui_engine.h` | `arex_left_widget_t` 移除 `font_id` 字段（与 protobuf 侧对齐）；`render_widget_by_id()` 函数签名增加 `arex_font_id_t cfg_font_id` 参数 |
| 2026-05-01 | `ui_engine.c` | `g_left_widgets[]` 初始化简化为 5 字段；`arex_render_5f_custom_grid()` 调用 `render_widget_by_id()` 删除多余参数 |
| 2026-05-01 | `data.c` | `arex_bus_set_ui_layout()` 删除 `font_id` 赋值行 |
...
| 2026-05-01 | `ui_engine.h` | 新增 `ELEM_TITLE|ELEM_VALUE|ELEM_UNIT|ELEM_BAR|ELEM_EXTRA` 8-bit 元素开关掩码宏（ELEM_EXTRA 用于 POD 专属 ID 标签）；`arex_widget_style_t` 结构体新增 `uint8_t elements` 字段 |
| 2026-05-01 | `ui_engine.c` | `g_widget_styles[]` 全部 30+ 组件条目新增 `.elements` 字段（严格按 PRD 约束表按需勾选）；`render_widget_by_id()` 重构为三段式流水线：专属早期分支（DEPTH/NDL）+ 通用掩码装配（ELEM_TITLE→ELEM_VALUE→ELEM_UNIT→ELEM_BAR→ELEM_EXTRA）+ 早期返回；删除废弃的旧通用渲染分支代码 |
| 2026-05-02 | `ui_engine.h` | 枚举重构：`WIDGET_WTIME_0806`（水面休息时间）已废弃；新增 `WIDGET_TIME_1606`（系统时间 ID=11）；移除废弃 ID：`TEMP_MAX_0806`(38)、`SAC_RATE_0806`(39)、`PPO2_SAFE`(50)、`NDL_SAFE`(51)、`SAC_SAFE`(52) |
| 2026-05-02 | `ui_engine.c` | `g_widget_styles[]` 废弃 ID 样式替换为 `WIDGET_EMPTY`；`g_left_widgets[2]` 改为 `WIDGET_DIVE_TIME_1606`；`render_widget_by_id()` 废弃 ID case 注释化 |
| 2026-05-02 | `screen.c` | `arex_screen_refresh_left_panel()` 新增 `WIDGET_DIVE_TIME_1606` case，使用 `arex_widget_set_text()` 配合 `snprintf` 格式化 `MM:SS`，而非 `arex_widget_set_value()`（会格式化为整数）；移除 `WIDGET_WTIME_0806` case |
...
| 2026-05-02 | `ui_engine.c` | 新增 `arex_widget_sync_data()` 全量组件数据路由分发器：覆盖全部 30+ 组件 ID，自动从 `g_sensor_data` 取值并调用 `arex_widget_set_value()` 或 `arex_widget_set_text()` 刷新界面 |
| 2026-05-02 | `ui_engine.h` | 新增 `arex_widget_sync_data()` 函数声明 |
| 2026-05-02 | `screen.c` | 新增 `arex_screen_refresh_all_widgets()` 统一全屏刷新接口；同时遍历 `g_left_widgets[]` 和 `g_5f_widgets[]`，内部调用 `arex_widget_sync_data()`；保留 `arex_screen_refresh_left_panel()` 兼容旧接口 |
| 2026-05-02 | `screen.h` | 新增 `arex_screen_refresh_all_widgets()` 函数声明 |
...
**移植步骤**：将 `data.h` 中 `#define PC_SIMULATOR` 整行注释或删除，然后引入 `#include <rtthread.h>`，即可切换到真机模式。`data.c` 和所有调用方**无需修改任何代码**，临界区自动启用。
...
在 **DIVE MENU** 卡片中新增 **LIGHT CONTROL** 选项，支持：
- 灯光开关控制（ON/OFF）
...
```c
/* screen.c - 全局灯光状态（供 LIGHT CONTROL 子菜单共享） */
...
| 2026-04-30 | `screen.c` | `s_setup_sub[]` 新增 LIGHT 子菜单配置 |
...
| 2026-04-30 | `screen.c` | 点击颜色选项时自动开灯逻辑 |
...
原因是 `s_ndl_handles[]` 在 `ui_engine.c` 中声明为 `static`，只能在那个文件内部访问。`screen.c` 无法直接访问。

**解决方案**：把这些共享数组的声明从 `static` 改为非 static，并移动到 `ui_engine.h` 头文件中作为 `extern` 声明：

```c
// ui_engine.h（头文件：extern 声明）
...
// screen.c（通过 include 访问）
#include "ui_engine.h"  // 隐式获得 extern 声明
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-05-01 | `ui_engine.h` | `ndl_handle_t` 结构体 + `s_ndl_handles[]` / `s_img_ascent_rate[]` 改为 extern 声明 |
| 2026-05-01 | `screen.c` | 新增 `clear_widget_arrays()` 统一清空函数 |
| 2026-05-01 | `screen.c` | `arex_screen_rebuild_layout()` 入口调用 `clear_widget_arrays()` |
| 2026-05-01 | `screen.c` | `left_anchor_create()` 入口调用 `clear_widget_arrays()` |
| 2026-05-01 | `ui_engine.c` | `arex_render_left_anchor_grid()` 移除自己的 memset |
| 2026-05-01 | `ui_engine.c` | `arex_render_5f_custom_grid()` 移除自己的 memset |
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-05-01 | `ui_engine.h` | 删除 `/* 向后兼容别名字典 (Legacy Aliases) */` 整块 #define 宏（约50行）；新增别名定义 `#define AREX_WIDGET_ASCENT_0812 WIDGET_ASCENT_0812` 等 |
| 2026-05-01 | `ui_engine.c` | 全局替换全部 AREX_WIDGET_* 别名为 WIDGET_*_<WH> 枚举值（共51处，分布在 ~60行代码中）；修复双后缀污染（`WIDGET_TEMP_0806_0806` → `WIDGET_TEMP_0806` 等24处） |
| 2026-05-01 | `ui_engine.c` | `g_widget_styles[]`（36个组件含 WIDGET_DEPTH_1606）；POD 渲染状态机（`s_pod_render_count`） |
| 2026-05-01 | `ui_engine.c` | `arex_get_widget_style()`、`arex_reset_widget_render_state()` 实现 |
| 2026-05-01 | `screen.c` | 全局替换 AREX_WIDGET_* 别名为 WIDGET_*_<WH>（12处） |
...
```c
/* ui_engine.h — arex_widget_id_t 枚举 */
typedef enum {
    /* ... 其他组件 ... */
    WIDGET_HEADING_0806  = 32,
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-05-01 | `ui_engine.h` | 删除 `WIDGET_POD1_VIRTUAL`、`WIDGET_POD2_VIRTUAL` 枚举；整理分组 |
| 2026-05-01 | `ui_engine.c` | 新增 `POD_TAG_BASE`、`POD1_TAG`、`POD2_TAG` 宏定义 |
| 2026-05-01 | `ui_engine.c` | 删除 `arex_get_widget_style()` 中的虚拟 ID 重定向逻辑 |
| 2026-05-01 | `ui_engine.c` | 重构 `render_widget_by_id()` 内部 POD 拦截逻辑 |
| 2026-05-01 | `ui_engine.c` | 重构 `arex_widget_set_value()` POD 精确靶向逻辑 |
| 2026-05-01 | `ui_engine.c` | 删除 `arex_render_widget()` 中的旧版虚拟 ID 路由 |
| 2026-05-01 | `AREX_ARCH.md` | 替换 Section 38.x 为新 POD 单模具架构文档 |
| 2026-05-02 | `ui_engine.c` | `g_widget_styles[]` 中 `WIDGET_POD_0806` 删除 `ELEM_EXTRA` 开关，只保留 `ELEM_TITLE \| ELEM_VALUE \| ELEM_UNIT` |
| 2026-05-02 | `ui_engine.c` | 删除 `render_widget_by_id()` 中的 `/* --- 零件 5：EXTRA 附加异构元素 --- */` 代码块 |
| 2026-05-02 | `ui_engine.c` | `arex_widget_set_value()` POD 更新逻辑简化为通过 `child_tag == WIDGET_POD_0806` 直接匹配 |
| 2026-05-02 | `AREX_ARCH.md` | Section 38.11 新增本次 POD 标题文本合并修复记录 |
| 2026-05-02 | `ui_engine.h` | `render_widget_by_id()` 删除 `is_depth_icon` 参数，改为工厂自主查字典决定 |
| 2026-05-02 | `ui_engine.c` | `render_widget_by_id()` 删除 `is_depth_icon` 参数，内部使用 `needs_bar_icon = (style->elements & ELEM_BAR)` 判断 |
| 2026-05-02 | `ui_engine.c` | 清理所有上层调用者（5F 网格、左侧锚点、arex_render_widget）中的 is_depth 参数 |
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-05-02 | `ui_engine.h` | `render_widget_by_id()` 删除 `is_depth_icon` 参数 |
| 2026-05-02 | `ui_engine.c` | 函数实现删除 `is_depth_icon` 参数，内部改用 `needs_bar_icon = (style->elements & ELEM_BAR)` |
| 2026-05-02 | `ui_engine.c` | 清理 `arex_render_5f_custom_grid()` 中的 false 参数 |
| 2026-05-02 | `ui_engine.c` | 清理 `arex_render_left_anchor_grid()` 中的 is_depth 变量和参数 |
| 2026-05-02 | `ui_engine.c` | 清理 `arex_render_widget()` 中的 is_depth_icon 判断 |
...
```c
// ui_engine.c — g_widget_styles[] 中 WIDGET_POD_0806 配置
{
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-05-02 | `ui_engine.c` | `g_widget_styles[]` 中 `WIDGET_POD_0806` 删除 `ELEM_EXTRA` 开关 |
| 2026-05-02 | `ui_engine.c` | 删除 `render_widget_by_id()` 中的 `/* --- 零件 5：EXTRA 附加异构元素 --- */` 代码块 |
| 2026-05-02 | `ui_engine.c` | `arex_widget_set_value()` POD 更新逻辑简化 |
...
| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-05-05 | `ui_engine.c` | 新增 `s_last_alarm_target` 静态变量保存清除前的目标 ID |
| 2026-05-05 | `ui_engine.c` | `arex_clear_all_alarm_styles()` 中先保存 `s_last_alarm_target` 再清空 |
| 2026-05-05 | `ui_engine.c` | 恢复逻辑改用 `s_last_alarm_target` 精确匹配，避免误伤分割线 |
| 2026-05-05 | `AREX_ARCH.md` | 新增 Section 43，记录本次告警恢复逻辑修复 |

---

## 44. GAS 界面双高亮问题修复 (v2026-05-06)

### 44.1 问题描述

**现象**：在 GAS 界面选中后去选择其他气体时，原本的气体仍然高亮，导致出现两个高亮。

**根因**：`card_gas_update()` 函数中，高亮条件为：

```c
bool highlight = is_cursor || is_active;
```

这意味着只要是当前激活的气体（`is_active`）就会高亮，同时光标所在行也会高亮。当处于 `UI_EDIT_GAS` 状态选择其他气体时，两个条件同时满足，导致出现两个高亮。

### 44.2 修复方案

**核心思想**：在 `UI_EDIT_GAS` 状态下，只有光标位置应该高亮，激活气体应该恢复普通样式。
...
| 状态 | 修复前 | 修复后 |
|------|--------|--------|
| `UI_DASH`（普通浏览） | 当前激活气体高亮 | 当前激活气体高亮 ✅ |
| `UI_EDIT_GAS`（选择中） | 激活气体 + 光标行同时高亮 ❌ | 只有光标行高亮 ✅ |

### 44.4 变更文件清单

| 日期 | 文件 | 变更 |
|------|------|------|
| 2026-05-06 | `card_gas.c` | `card_gas_update()` 中高亮条件改为 `is_cursor \|\| (is_active && g_ui.state != UI_EDIT_GAS)` |
| 2026-05-06 | `AREX_ARCH.md` | 新增 Section 44，记录 GAS 界面双高亮问题修复 |
