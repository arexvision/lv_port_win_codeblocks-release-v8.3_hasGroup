...
>
> **2026-05-03 更新 #3**：`arex_ui_engine.h` 移除重复注释
...
| 2026-04-28 | `arex_ui_engine.h` | `WIDGET_GAS_1606` 新增；`WIDGET_WTIME_0806` 注释改为 DIVE_TIME，数据源改为 `dive_time_s` |
| 2026-04-28 | `arex_ui_engine.c` | `s_widget_meta[]` 新增 GAS 条目；WTIME title 改为 "DIVE" |
...
| 2026-05-02 | `arex_ui_engine.h` | 枚举重构：`WIDGET_WTIME_0806`（水面休息时间）已废弃；新增 `WIDGET_TIME_1606`（系统时间 ID=11）；移除废弃 ID：`TEMP_MAX_0806`(38)、`SAC_RATE_0806`(39)、`PPO2_SAFE`(50)、`NDL_SAFE`(51)、`SAC_SAFE`(52) |
| 2026-05-02 | `arex_ui_engine.c` | `g_widget_styles[]` 废弃 ID 样式替换为 `WIDGET_EMPTY`；`g_left_widgets[2]` 改为 `WIDGET_DIVE_TIME_1606`；`render_widget_by_id()` 废弃 ID case 注释化 |
| 2026-05-02 | `arex_screen.c` | `arex_screen_refresh_left_panel()` 新增 `WIDGET_DIVE_TIME_1606` case，使用 `arex_widget_set_text()` 配合 `snprintf` 格式化 `MM:SS`，而非 `arex_widget_set_value()`（会格式化为整数）；移除 `WIDGET_WTIME_0806` case |
