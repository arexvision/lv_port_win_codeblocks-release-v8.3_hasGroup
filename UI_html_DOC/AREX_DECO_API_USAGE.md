...
- 主图是纯黑背景上的竖向 16 根组织柱，当前用单色绿色 G 通道亮度区分区域。
- 400 横线是环境压力线，颜色 `#007F00`；900 横线是 M 值线，颜色 `#00FF00`；PI 横向虚线来自 `tissue_pi_permille`，颜色 `#003300`。
- `PI`、`AMB`、`M` 文字标签由 `card_deco.c` 顶部宏控制；宏只影响文字显示，不影响归一化计算和参考线。
- 每根组织柱直接使用 `tissue_bar_permille[i]` 作为高度；400 以下使用 `#004C00`，400~900 使用 `#00CC00`，超过 900 的部分使用 `#00FF00` 闪烁。
