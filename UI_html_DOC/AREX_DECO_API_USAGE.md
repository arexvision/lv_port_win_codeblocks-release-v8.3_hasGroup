...
- 主图是纯黑背景上的横向 16 行组织条，当前完全剥离色相差异，使用绿色 G 通道亮度阶梯表达状态。
- 400 竖线是环境压力线，使用 `#007F00`；900 竖线是 M 值线，使用 `#00FF00`；PI 虚线来自 `tissue_pi_permille`，使用 `#003300`。
- `PI`、`PAMB`、`M` 文字标签由 `card_deco.c` 绘制；文字只用于标注，不影响归一化计算和参考线。
- 每根组织条直接使用 `tissue_bar_permille[i]` 作为长度；400 以下安全段使用 `#004C00`；400~900 排氮段使用 `#00CC00`；超过 900 的部分使用 `#00FF00` 满亮闪烁。
