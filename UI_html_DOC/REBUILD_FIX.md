# screen_rebuild_layout 修复记录

## 修复状态: 已完成 ✓

## 修复总览

本次修复解决了 `screen_rebuild_layout()` 和 `screen_rebuild_tileview()` 函数中的 4 个关联问题，这些问题导致布局切换时出现对象丢失、焦点丢失、崩溃和卡死现象。
...
### 根因分析
`screen_rebuild_tileview()` 函数中存在重复调用链：
...
### 修复方案
删除 `screen_rebuild_tileview()` 末尾重复的 `wall_create()`、`submenu_layer_create()`、`modal_create()` 调用，因为这三个函数已经在 `right_panel_create()` 内部被正确调用了。

**修复代码位置**: `src/ui/screen.c` 第 517-519 行
...
### 根因分析
`screen_rebuild_tileview()` 删除并重建了整个 `s_right_cont`（包含 tileview），但没有保存和恢复当前 tile 的焦点状态。重建后 LVGL 默认显示第一个 tile，而不是用户之前选中的位置。

### 修复方案
1. 在删除对象前保存当前焦点位置（使用 `g_ui.dash_card`，它已经保存了当前卡片位置）
2. 重建后在 `s_tileview` 上调用 `lv_obj_set_tile()` 恢复焦点

**修复代码位置**: `src/ui/screen.c` 第 481-522 行
...
### 根因分析
`screen_rebuild_tileview()` 删除 `s_right_cont` 时，会触发 LVGL 的延迟删除机制（对象在下一帧渲染时才真正释放）。在对象真正释放完成前：

1. `s_tileview` 指针已被置 NULL
2. 其他代码路径（如 `wall_nudge_tileview()`、`screen_show_wall()` 等）仍然持有老的 `s_tileview` 指针副本
3. 旋转时会调用 `screen_hide_walls()` 等函数，这些函数直接访问 `s_tileview` 而没有 NULL 检查
...
| `screen_show_wall()` | 第 832 行 |
| `screen_hide_walls()` | 第 851 行 |
| `screen_hide_walls_snap()` | 第 871 行 |
| `screen_scroll_to_card()` | 第 704 行 |
...
| 函数名 | 修复位置 |
|--------|----------|
| `screen_rebuild_layout()` | 第 429-432 行 |
| `screen_rebuild_tileview()` | 第 475-478 行 |
...
static void sim_tick_cb(lv_timer_t *t)
{
    ...
    if (s_layout_tick % 1 == 0) {  /* 每秒触发一次 */
        ...
        test_set_ui_layout(s_layout_phase);
...
### 根因分析
`screen_rebuild_tileview()` 在删除并重建 tileview 时：
