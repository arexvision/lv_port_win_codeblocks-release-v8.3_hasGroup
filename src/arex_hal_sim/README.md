...
| `sim_data.h/c` | 创建模拟 tick；可通过 `AREX_TCP_ALGO_DEBUG` 在原版自动模拟和 TCP 算法调试之间切换。 |
...
- `AREX_TCP_ALGO_DEBUG=1` 时，模拟 tick 在 TCP client 连接前不推进数据；连接成功后触发一次调试重置，再用 TCP 深度驱动 Buhlmann 调试算法。
- `AREX_TCP_ALGO_DEBUG=0` 时，不启动 TCP 算法调试，恢复原版自动深度脚本和假数据模拟。
