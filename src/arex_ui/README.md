...
```text
硬件 / BLE / PC 仿真数据
  -> core/arex_data.c: arex_bus_set_*()
  -> g_sensor_data / g_sys_config + dirty_mask
  -> core/arex_ui_engine.c: arex_ui_update_task()
  -> core/arex_ui_update_router.c
...
- 外部数据写入统一走 `core/arex_data.c` 的 `arex_bus_set_*()`，不要在其他模块直接写 `g_sensor_data` 或 `g_sys_config`。
- 屏幕级容器和导航放在 `screen/`，具体小组件内容放在 `comp/`，右侧整页内容放在 `cards/`。
- 卡片创建和刷新由 `screen/arex_card_registry.c` 统一登记；新增卡片时不要只新增 `card_*.c`。
