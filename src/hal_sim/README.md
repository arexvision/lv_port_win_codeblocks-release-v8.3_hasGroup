...
- TCP 指令支持 `heading <deg> [on|off]`、`heading off`、`compass_lock <deg|current|off>` 和 `compass_cal run [duration_ms] [hint]`，用于模拟指南针可用/不可用、目标锁定和 0~100 校准动画；固定帧调试可用 `compass_cal set <idle|run|saving|verify|ready|save_error|error> [progress] [bins] [hint]`。
