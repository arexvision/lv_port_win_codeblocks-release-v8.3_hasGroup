...
- TCP 指令支持 `heading <deg> [on|off]`、`heading off`、`compass_lock <deg|current|off>` 和 `compass_cal <idle|run|saving|verify|ready|save_error|error> [progress] [bins] [hint]`，用于模拟指南针可用/不可用、目标锁定和校准流程。
