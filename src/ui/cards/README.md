...
`cards` 放右侧 tileview 里的独立业务卡片。这里的文件通常直接负责一整页业务 UI 的创建和刷新，再由 `screen/page_registry.c` 登记到右侧页面序列。
...
| `card_plan.c` | 潜水轨迹图和减压站预测图；减压站序列由 `../core/data.c` 持有，卡片只读取绘制。 |
...
- 新增业务卡片：新增卡片实现后，还要更新 `screen/page_registry.h/c` 和默认页面顺序。
