...
static lv_coord_t tissue_y_for_permille(const lv_area_t *plot, int permille)
{
    int draw_permille = tissue_draw_permille_for_range(permille);
    int plot_h = lv_area_get_height(plot) - 1;
    return plot->y2 - (lv_coord_t)((draw_permille * plot_h) / TISSUE_UI_MAX_PERMILLE);
}

static void tissue_draw_horizontal_line(lv_draw_ctx_t *draw_ctx, const lv_area_t *plot, int permille, lv_color_t color, lv_opa_t opa, lv_coord_t width, lv_coord_t dash_width, lv_coord_t dash_gap)
{
...
static void tissue_draw_bar_segment(lv_draw_ctx_t *draw_ctx, lv_draw_rect_dsc_t *rect_dsc, const lv_area_t *plot, lv_coord_t x1, lv_coord_t x2, int low_permille, int high_permille, lv_color_t color, lv_opa_t opa)
{
    ...
    tissue_draw_rect(draw_ctx, rect_dsc, x1, tissue_y_for_permille(plot, draw_high), x2, tissue_y_for_permille(plot, draw_low), color, opa);
}
...
static void tissue_chart_draw_cb(lv_event_t *e)
{
    ...
    tissue_draw_bar_segment(draw_ctx, &rect_dsc, &plot, plot.x1, plot.x2, 0, TISSUE_UI_PAMB_PERMILLE, lv_color_make(0x00, 0x22, 0x00), LV_OPA_COVER);
    tissue_draw_bar_segment(draw_ctx, &rect_dsc, &plot, plot.x1, plot.x2, TISSUE_UI_PAMB_PERMILLE, TISSUE_UI_MVALUE_PERMILLE, lv_color_make(0x26, 0x22, 0x00), LV_OPA_COVER);
    tissue_draw_bar_segment(draw_ctx, &rect_dsc, &plot, plot.x1, plot.x2, TISSUE_UI_MVALUE_PERMILLE, TISSUE_UI_MAX_PERMILLE, lv_color_make(0x28, 0x00, 0x00), LV_OPA_COVER);

    plot_w = lv_area_get_width(&plot);
    for (int i = 0; i < TISSUE_COMPARTMENT_COUNT; i++)
    {
        lv_coord_t col_x1 = plot.x1 + (lv_coord_t)((i * plot_w) / TISSUE_COMPARTMENT_COUNT);
        lv_coord_t col_x2 = plot.x1 + (lv_coord_t)((((i + 1) * plot_w) / TISSUE_COMPARTMENT_COUNT) - 1);
        lv_coord_t bar_x1 = (lv_coord_t)(col_x1 + 2);
        ...
        tissue_draw_bar_segment(draw_ctx, &rect_dsc, &plot, bar_x1, bar_x2, 0, value_permille < TISSUE_UI_PAMB_PERMILLE ? value_permille : TISSUE_UI_PAMB_PERMILLE, GREEN, LV_OPA_COVER);
        if (value_permille > TISSUE_UI_PAMB_PERMILLE) tissue_draw_bar_segment(draw_ctx, &rect_dsc, &plot, bar_x1, bar_x2, TISSUE_UI_PAMB_PERMILLE, value_permille < TISSUE_UI_MVALUE_PERMILLE ? value_permille : TISSUE_UI_MVALUE_PERMILLE, lv_color_make(0xFF, 0xD0, 0x00), LV_OPA_COVER);
        if (value_permille > TISSUE_UI_MVALUE_PERMILLE && s_tissue_flash_phase) tissue_draw_bar_segment(draw_ctx, &rect_dsc, &plot, bar_x1, bar_x2, TISSUE_UI_MVALUE_PERMILLE, value_permille, lv_color_make(0xFF, 0x20, 0x20), LV_OPA_COVER);
    }

    tissue_draw_horizontal_line(draw_ctx, &plot, TISSUE_UI_PAMB_PERMILLE, GREEN, LV_OPA_COVER, 2, 0, 0);
    tissue_draw_horizontal_line(draw_ctx, &plot, TISSUE_UI_MVALUE_PERMILLE, lv_color_make(0xFF, 0x20, 0x20), LV_OPA_COVER, 2, 0, 0);
    if (s_deco_vm_cache.tissue_normalized_valid != 0U)
    {
