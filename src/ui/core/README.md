...
| `callbacks.h/c` | UI 调业务层的默认回调，例如亮度、灯光、保守度、时间、蓝牙和报警设置。 |
...
- 新增传感器字段：先看 `ui_engine.h` 的数据结构，再补 `data.h/c` 写入接口和 dirty 标记。
- 修改旋钮/按键行为：看 `ui_state.c`，不要把输入分支塞进卡片文件。
- 修改刷新频率或 dirty 分发：看 `update_router.c`。
- 对接真机业务回调：看 `callbacks.h/c` 的接口边界。
