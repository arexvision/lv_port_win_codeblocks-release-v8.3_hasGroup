...
| `submenu_model.h/c` | 兼容模型：动态文案、复杂编辑流程、DIVE PLAN 渲染数据和部分旧设置后端。新菜单业务优先迁到 `menu_defs/runtime/actions`。 |
...
- 改 DIVE PLAN、内联数值编辑或旧兼容动态文案：看 `submenu_model.c`。
