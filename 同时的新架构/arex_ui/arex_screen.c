...
static const char *card_id_name(uint8_t card_id)
{
    switch (card_id) {
    case CARD_ID_INFO:
        return "INFO";
    case CARD_ID_COMPASS:
        return "COMPASS";
    case CARD_ID_DECO:
        return "DECO";
    case CARD_ID_GAS:
                                           ...
                                           bool selected)
{
    ...
    if (badge_lbl) {
        lv_obj_set_style_text_color(badge_lbl, selected ? AREX_LIGHT : AREX_LIGHT, 0);
    }
}
...
static void right_panel_create(void)
{
    ...
    for (uint8_t i = 0; i < count; i++) {
        ...
        uint8_t card_id = arex_card_id_at(i);
        if (!card) continue;
        ...
        switch (card->engine_type) {
            case CARD_ENGINE_GRID: {
                ...
                uint8_t custom_card_idx = (storage_pos < AREX_CARD_COUNT)
                    ...
                    : 0xFF;
                LOG_I("[TILE_CREATE] pos=%u card=%s(%u) engine=GRID custom_idx=%u widget_count=%u",
                      i,
                      card_id_name(card_id),
                      card_id,
                      custom_card_idx,
                      (custom_card_idx < AREX_MAX_CUSTOM_CARDS)
                          ? g_sys_config.custom_cards[custom_card_idx].widget_count
                          : 0);
                if (custom_card_idx < AREX_MAX_CUSTOM_CARDS) {
                    arex_render_5f_custom_grid(tile, g_left_anchor_obj, custom_card_idx);
                } else {
                    LOG_W("[TILE_CREATE] pos=%u CUSTOM 槽位未绑定有效 custom_card_idx", i);
                }
                break;
            }
            ...
            default:
                LOG_I("[TILE_CREATE] pos=%u card=%s(%u) engine=%u custom_idx=%d",
                      i,
                      card_id_name(card_id),
                      card_id,
                      card->engine_type,
    ...
    LOG_I("[TILE_CREATE] dash_slots=%u custom_card_count=%u",
          arex_visible_dash_count(),
          g_sys_config.custom_card_count);
...
void arex_screen_scroll_to_card(uint8_t tile_pos)
{
    ...
    LOG_I("[SCROLL] tile_pos=%u card=%s(%u) tile=%p",
          tile_pos,
          card_id_name(arex_card_id_at(tile_pos)),
          arex_card_id_at(tile_pos),
          tile);
...
void arex_screen_refresh_all_widgets(void)
{
    ...
    /* 2. 同步右侧多张自定义区配置 */
    for (uint8_t card = 0; card < g_sys_config.custom_card_count && card < AREX_MAX_CUSTOM_CARDS; card++) {
        for (uint8_t i = 0; i < g_sys_config.custom_cards[card].widget_count && i < AREX_5F_MAX_WIDGETS; i++) {
            if (g_sys_config.custom_cards[card].widgets[i].widget_id != WIDGET_EMPTY) {
                arex_widget_sync_data(g_sys_config.custom_cards[card].widgets[i].widget_id);
            }
        }
    }
}
...
static const char *s_setup_sub[][7] = {
    ...
    { "MIN", "LOW", "MED", "HIGH", "ULTRA", "MAX", "< BACK" },  // 6 档亮度
