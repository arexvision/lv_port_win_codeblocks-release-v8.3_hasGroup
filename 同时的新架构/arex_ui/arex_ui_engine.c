...
/* DEPTH 2x2 子标签烙印，避免更新阶段依赖 child 下标导致串位 */
#define DEPTH_INT_TAG  ((uintptr_t)0xD201)
#define DEPTH_DEC_TAG  ((uintptr_t)0xD202)
 ...
 * ========================================================= */
    /* ========== 核心驻留组件 ========== */
    {
        ...
        .spec.depth = {
            /* 左侧主数值与右侧速率图标彻底分舱，避免小数/单位压到图片区域 */
            .int_offset_x = -92, .int_offset_y = 10, .int_align = LV_ALIGN_RIGHT_MID,
            .dec_offset_x = 4,  .dec_offset_y = -26,
            // 单位挂在小数的正下方
            .unit_offset_x = 0, .unit_offset_y = 0,
            .icon_offset_x = -4, .icon_offset_y = 0, .icon_align = LV_ALIGN_RIGHT_MID
        }
    },
    {
...
void arex_sys_config_defaults(arex_sys_config_t *cfg)
{
    ...
    cfg->custom_card_slot[CARD_POS_DYNAMIC_FIRST] = 0;
    ...
    cfg->custom_cards[0].widgets[3]  = (arex_grid_widget_t){ WIDGET_EMPTY,          0, 2 };
    ...
    cfg->custom_cards[0].widgets[11] = (arex_grid_widget_t){ WIDGET_EMPTY,          4, 4 };
     ...
     * 中间动态槽位数量由 AREX_MAX_DYNAMIC_SLOTS 控制。
     */
    for (int i = 0; i < AREX_CARD_COUNT; i++) {
        cfg->card_order[i] = CARD_ID_BLANK;
    }
    cfg->card_order[CARD_POS_INFO]          = CARD_ID_INFO;
    cfg->card_order[CARD_POS_DYNAMIC_FIRST] = CARD_ID_CUSTOM_GRID;
    cfg->card_order[CARD_POS_2]             = CARD_ID_COMPASS;
    cfg->card_order[CARD_POS_3]             = CARD_ID_DECO;
    cfg->card_order[CARD_POS_4]             = CARD_ID_PLAN;
    cfg->card_order[CARD_POS_5]             = CARD_ID_GAS;
...
lv_obj_t *g_card_custom_objs[AREX_MAX_CUSTOM_CARDS] = {0};
uint8_t   g_card_custom_obj_count = 0;
...
static arex_widget_id_t s_alarm_target_id = WIDGET_EMPTY;
static arex_alarm_level_t s_alarm_target_level = AREX_ALARM_NONE;

/* 告警横幅静态样式（避免每次创建重复初始化） */
static lv_style_t s_banner_style_warn;
static lv_style_t s_banner_style_crit;

static void arex_hide_alarm_banner(void);

static void alarm_apply_style_to_widget(lv_obj_t *obj, bool active, arex_alarm_level_t level)
{
    if (!obj) return;

    lv_color_t bg_color = active ? AREX_LIGHT : AREX_BLACK;
    lv_color_t text_color = active ? AREX_BLACK : AREX_GREEN;
    lv_opa_t bg_opa = active ? LV_OPA_COVER : LV_OPA_TRANSP;

    lv_obj_set_style_bg_color(obj, bg_color, 0);
    lv_obj_set_style_bg_opa(obj, bg_opa, 0);

    int16_t child_cnt = lv_obj_get_child_cnt(obj);
...
static void alarm_apply_style_to_target_widgets(bool active)
{
    if (s_alarm_target_id == WIDGET_EMPTY) return;
    ...
    for (uint8_t c = 0; c <= max_count; c++) {
        ...
        for (int16_t i = 0; i < child_cnt; i++) {
            ...
            if ((arex_widget_id_t)(uintptr_t)lv_obj_get_user_data(child) == s_alarm_target_id) {
                alarm_apply_style_to_widget(child, active, s_alarm_target_level);
            }
        }
    }
}
                              ...
                              arex_font_id_t cfg_font_id)
{
    ...
    if (w_id == WIDGET_DEPTH_1612 && is_2x2) {
        ...
        lv_obj_set_user_data(int_lbl, (void *)DEPTH_INT_TAG);
        ...
        if (AREX_SHOW_PLACEHOLDER_ON_INIT) {
            lv_label_set_text(dec_lbl, ".--");  // 两位小数占位符
        } else {
            /* 修复：与更新函数保持一致，使用两位小数 + 溢出保护 */
            float local_depth = g_sensor_data.depth;
            if (local_depth < 0.0f) local_depth = 0.0f;  // 负数保护
            int di = (int)local_depth;
            int dd = (int)((local_depth - di) * 100 + 0.5f);
            if (dd >= 100) { di++; dd = 0; }  // 溢出保护
            lv_label_set_text_fmt(dec_lbl, ".%02d", dd);
        }
        ...
        lv_obj_set_user_data(dec_lbl, (void *)DEPTH_DEC_TAG);
        ...
        /* ==========================================
         * 3. 单位 (m) -> 紧贴小数正下方
         * 同事要求：改为更大的字体（16×12 样式 → 20px TITLE）
         * ========================================== */
        if (style->elements & ELEM_UNIT) {
            lv_obj_t *unit_lbl = lv_label_create(obj);
            lv_label_set_text(unit_lbl, style->unit ? style->unit : "");
            // 单位改为 TITLE 字体（20px），比原来的 SMALL（14px）更大
            lv_obj_set_style_text_font(unit_lbl, arex_get_font(AREX_FONT_ID_TITLE), 0);
...
static void render_custom_card_widgets(lv_obj_t *card_custom, uint8_t custom_card_idx)
{
    ...
    for (uint8_t i = 0; i < count; i++) {
        ...
        LOG_I("[CUSTOM_RENDER] card=%u widget[%u]=%u rc=(%u,%u) span=%ux%u rect=(%d,%d,%u,%u)",
              custom_card_idx,
              i,
              w_id,
              r,
    ...
    for (uint8_t c = 0; c <= max_count; c++) {
        ...
        for (int16_t i = 0; i < child_cnt; i++) {
            ...
            if (id == WIDGET_DEPTH_1612 && child_tag == (uintptr_t)id) {
                /* 修复 Bug #1 & #2：
                 * 1. 原子读取一次 value，避免数据竞争
                 * 2. 改为两位小数显示（*100）
                 * 3. 处理小数溢出（dd >= 100 时进位）
                 * 4. 处理负数深度（传感器噪声保护）
                 */
                float local_value = value;  // 原子读取，避免两次读取不一致
                ...
                int di = (int)local_value;
                int dd = (int)((local_value - di) * 100 + 0.5f);

                /* 小数溢出保护：当 dd >= 100 时进位到整数部分 */
                if (dd >= 100) {
                    di++;
                    dd = 0;
                }

                lv_obj_t *int_lbl = NULL;
                lv_obj_t *dec_lbl = NULL;
                ...
                for (int16_t j = 0; j < sub_cnt; j++) {
                    ...
                    uintptr_t sub_tag = (uintptr_t)lv_obj_get_user_data(sub);
                    if (sub_tag == DEPTH_INT_TAG) {
                        int_lbl = sub;
                    } else if (sub_tag == DEPTH_DEC_TAG) {
                        dec_lbl = sub;
                    }
                }

                if (int_lbl && lv_obj_check_type(int_lbl, &lv_label_class)) {
                    lv_label_set_text_fmt(int_lbl, "%d", di);
                }
                if (dec_lbl && lv_obj_check_type(dec_lbl, &lv_label_class)) {
                    lv_label_set_text_fmt(dec_lbl, ".%02d", dd);  // %02d 确保补零
                }
                break;
            }
...
static void alarm_blink_cb(lv_timer_t *timer)
{
    ...
    alarm_apply_style_to_target_widgets(s_alarm_blink_on);

    if (s_alarm_banner && s_alarm_banner_lbl) {
        lv_obj_set_style_bg_color(s_alarm_banner, s_alarm_blink_on ? AREX_LIGHT : AREX_BLACK, 0);
        lv_obj_set_style_bg_opa(s_alarm_banner, LV_OPA_COVER, 0);
        lv_obj_set_style_text_color(s_alarm_banner_lbl, s_alarm_blink_on ? AREX_BLACK : AREX_GREEN, 0);
    }
}
...
{
    s_alarm_target_id = WIDGET_EMPTY;
    s_alarm_target_level = AREX_ALARM_NONE;
    ...
    s_alarm_level = level;
    s_alarm_target_id = target_id;
    s_alarm_target_level = level;

    if (eng_text) {
        snprintf(s_alarm_text, sizeof(s_alarm_text), "%s", eng_text);
    } else {
        s_alarm_text[0] = '\0';
    }
    ...
    alarm_apply_style_to_target_widgets(true);
...
void arex_clear_all_alarm_styles(void)
{
    ...
    alarm_apply_style_to_target_widgets(false);
...
void arex_show_alarm_banner(arex_alarm_level_t level, const char *eng_text)
{
    ...
    int right_canvas_w = (int)g_sys_config.safe_zone_w - (int)AREX_LEFT_ANCHOR_W
                       - (int)(g_sys_config.panel_gap_u * AREX_BASE_U);
    if (right_canvas_w <= 0) {
        right_canvas_w = AREX_PHYSICAL_W;
    }
    ...
    if (!s_alarm_banner) {
        ...
        lv_obj_set_style_text_font(s_alarm_banner_lbl, arex_get_font(AREX_FONT_ID_TITLE), 0);
        lv_obj_set_style_text_color(s_alarm_banner_lbl, AREX_BLACK, 0);
        lv_obj_set_width(s_alarm_banner_lbl, right_canvas_w - 24);
        lv_obj_align(s_alarm_banner_lbl, LV_ALIGN_LEFT_MID, 12, 0);
        lv_label_set_long_mode(s_alarm_banner_lbl, LV_LABEL_LONG_DOT);
    }

    lv_obj_set_size(s_alarm_banner, right_canvas_w, 36);
    ...
    lv_obj_set_style_bg_color(s_alarm_banner, AREX_LIGHT, 0);
    lv_obj_set_style_bg_opa(s_alarm_banner, LV_OPA_COVER, 0);
...
#if AREX_ALARM_SHOW_PREFIX
    {
        ...
        if (level == AREX_ALARM_CRIT) prefix = "CRITICAL";
        else if (level == AREX_ALARM_WARN) prefix = "WARNING";
        snprintf(full_text, sizeof(full_text), "%s: %s", prefix, eng_text);
        lv_label_set_text(s_alarm_banner_lbl, full_text);
    }
#else
    lv_label_set_text(s_alarm_banner_lbl, eng_text);
#endif
}
