...
int Buhlmann::calculateMinutesRequiredToReachCertainPressure(float targetPressure) {
	...
	while (!stopTimeCalculation) {
		...
		if (firstDecoStop < DECO_FINAL_STOP_METERS) firstDecoStop = DECO_FINAL_STOP_METERS;
		float currentGF = calculateCurrentGF(currentDepthForGF, firstDecoStop, DECO_FINAL_STOP_METERS);
    ...
    if (timeSpentInLevel > 0) {
        ...
        if (!decoNeeded && !hasValidDecoSequence)
		{
            ...
            if (_decoSequence.currentStopIdx >= 0 && _decoSequence.currentStopIdx < _decoSequence.stopCount) {
                ...
                float nextStopDepth = (currentStop.depth <= DECO_FINAL_STOP_METERS)
                                      ? 0.0f
                                      : currentStop.depth - DECO_STEP_METERS;
...
void Buhlmann::generateDecoSequence(float currentPressure, float currentDepth) {
    ...
    for (int i = 1; i < decoStopCount; i++) {
        if (decoStopDepths[i] >= (int)DECO_FINAL_STOP_METERS) {
            finalStopIdx = i;
            break;
        }
    }
...
float Buhlmann::calculateFirstStopDepthWithGF(float currentDepth, float gfLow) {
    ...
    // 2) 向上取整到 3m 阶梯
    float firstStopDepth = DECO_FINAL_STOP_METERS;  // 最小首停深度
    if (ceilingDepthLow > DECO_FINAL_STOP_METERS) {
        // 向上取整到 3m 阶梯
        firstStopDepth = ceil(ceilingDepthLow / DECO_STEP_METERS) * DECO_STEP_METERS;
    }

    if (firstStopDepth < DECO_FINAL_STOP_METERS) {
        firstStopDepth = DECO_FINAL_STOP_METERS;
    }

    // 3) 基本边界保护：不能超过当前深度 - 3m
    float maxAllowedFirst = currentDepth - DECO_STEP_METERS;
    if (firstStopDepth > maxAllowedFirst) {
        if (maxAllowedFirst < DECO_FINAL_STOP_METERS) firstStopDepth = DECO_FINAL_STOP_METERS;
...
void Buhlmann::testGFCalculation() {
    ...
    float finalStop = DECO_FINAL_STOP_METERS;
